    ...
    <section aria-labelledby="order-review-title" className="space-y-4 pb-5">
      <div className="brand-panel-strong rounded-3xl p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-base font-extrabold text-brand-text" id="order-review-title">
          ...
          <article className="brand-list-item rounded-2xl px-4 py-3" key={line.key}>
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="break-words text-sm font-bold leading-5 text-brand-text">{line.label}</p>
                <p className="brand-text-subtle mt-1 text-xs">{formatWon(line.price)} × {line.quantity}</p>
              </div>
              <p className="shrink-0 text-sm font-extrabold text-brand-text">{formatWon(line.total)}</p>
            </div>
          </article>
        ))}
      </div>
      ...
      <dl className="brand-inset-panel space-y-2 rounded-3xl p-4 text-sm">
        ...
        <div className="flex justify-between gap-3"><dt className="brand-text-subtle">남은 지원 {formatWon(summary.supportRemaining)}</dt><dd className="font-bold text-brand-success-text">{summary.supportApplied > 0 ? `-${formatWon(summary.supportApplied)}` : formatWon(0)}</dd></div>
        <div className="flex justify-between gap-3 border-t border-brand-divider pt-2"><dt className="font-extrabold">청구금액</dt><dd className={`font-extrabold ${summary.billableAmount > 0 ? "text-brand-danger" : "text-brand-success-text"}`}>{summary.billableAmount > 0 ? formatWon(summary.billableAmount) : "없음"}</dd></div>
      </dl>
    </section>
  );
}
