...
- 공지는 헤더의 작은 종 아이콘으로만 열고, 별도 상단 배너는 쓰지 않는다.
- 관리자가 정산을 연 회차가 있으면 홈 진입 시 공지 팝업 안에서 바로 정산 내역을 보여주고, 사용자가 `정산 확인 완료`를 누르기 전까지 계속 다시 표시한다.
- 정산 팝업에는 관리자 설정의 입금 계좌를 함께 노출하고, 계좌번호 복사 버튼을 제공한다.
